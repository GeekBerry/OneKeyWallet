# OneKey Software Wallet

## 概念定义

 concept         | definition                                
-----------------|-------------------------------------------
 account_address | 链上账户地址, 如 evm 链指全小写 hex40                 
 token_address   | 链上代币地址                                    
 amount          | 最小单位的资产数量, 如 eth 单位为(wei), ERC1155 单位为(个) 
 decimals        | 以10为底指数精度                                 
 balance         | 金额, 通常为 amount/(10**decimals)             
 usd_price       | 单位金额主代币的美元值                               
 chain           | 链标识, 主网和测试网属于不同的 chain                    
 timestamp       | 以秒为单位的Unix时间戳                             
 block_number    | 区块编号整数                                    
 transaction     | 交易(事务)                                    
 transfer        | 代币转移                                      

------------------------------------------------------------

## 需求分析

### 主要需求分列如下:

* 主要支持 BTC, EVM, SOL, TRON 链
* 主币支持
    * 获取 account_address 当前主币 balance
    * 获取主币当前法币价值
* 同质化代币资产支持
    * 获取 account_address 当前同质化代币列表
    * 获取 account_address 当前同质化代币 balance
    * 获取同质化代币当前法币价值
* 非同质化代币资产支持
    * 获取 address 当前非同质化代币列表
    * 获取 address 当前非同质化代币 balance
    * 获取非同质化代币当前法币价值
* 交易历史支持
    * 获取 address 主币交易历史
    * 获取 address 同质化代币交易历史
    * 获取 address 非同质化代币交易历史

### 未来可能要求支持的需求:

*以下需求在本版本系统设计需求外, 但参考竞品设计后可预计, 很有可能在后续产品中要求支持, 在系统设计中应当给予考虑*

* 产品有同一链支持很多地址的需求可能: 如 BTC 链有 4 种类, 不同格式的地址, 一个 EOA 账户可能控制多个 AA 地址账户等
* 产品有同一页面展示所有链资产总和的需求可能:
* 产品有展示其他法币的需求可能:
* 产品有列表隐藏小额资产的需求可能: 这里的小额指法币价值低于一定数量
* 地址详细信息维护: 如 ENS, 风险地址标识, 通用地址标识(如 UniSwap 池子地址)等.

------------------------------------------------------------

## 概要设计

### 展示 address token balance 实现流程

* 通过 [token_data_table](./doc/实体关系表.md#代币信息储存表--tokendatatable) 维护经过删选的链上代币信息列表,
  并只展示该列表范围内的资产信息
* 为了快速获取 资产数据, 优先通过由服务端维护的
  [account_amount_cache_table](./doc/实体关系表.md#用户资产缓存表--accountamountcachetable) 获取 address 资产 amount.
* 为了数据准确性, 允许参数触发实时获取链上 address 对应 amount 数据.
* 通过 join [token_data_table](./doc/实体关系表.md#代币信息储存表--tokendatatable) 获取 address 资产 balance
* 通过 join [token_price_table](./doc/实体关系表.md#代币币价表--tokenpricetable) 获取 address资产 usd balance
* 对所有代币 usd balance 加总展示 address 资产总额度
* 资产查询结果可以加以缓存, 缓存时间通常为相应链的区块时间.

### 展示 address transfer 实现流程

* 通过 [token_data_table](./doc/实体关系表.md#代币信息储存表--tokendatatable) 维护经过删选的链上代币信息列表,
  并只展示该列表范围内的资产信息
* 对于大部分已经添加扫快信息的链, 通过 [token_transfer_table](./doc/实体关系表.md#资产转移表--tokentransfertable) 获取
  address 历史交易数据, (注意可选择 0 额转账过滤)
* 对于没有扫快信息的链, 允许支持通过第三方数据源获取历史交易数据.
* 转账历史展示中, 由于可能动态过滤某些项目(如 0 金额), 因此使得计算总数和随机翻页比较困难,
  不要设计转账总数和页码翻页, 要支设计支持通过游标翻页, 可能要支持按时间筛选范围.
* 因为同一 transaction 可能产生多笔 transfer, 在展示历史条目时, 需要对 transfer 进行聚合.
  几个聚合方案:
    * 找出同一 transaction 下的所有与 address 相关的 transfer.
    * 消除 0 额代币转移, 如同一地址下两笔 1ETH -> 1WETH, 1WETH -> 2000USDT, 应当 WETH 转移金额为 0
    * 扣除回款代币转移, 如BTC下 output:[3,2], input[2], 则实际代币花费为 (3+2)-2 = 3.
    * 对于多输入单多输出的交易, 可以简单聚合会地址 address 在这笔交易中共计花出或收入数量, 而不计对手地址.

### 维护 token_data_table 实现流程

* [token_data_table](./doc/实体关系表.md#代币信息储存表--tokendatatable) 用于管理经过钱包验证的代币信息
* 代币信息可以设计管理员接口进行维护
* 代币信息的 name, symbol, decimals 等项, 可以支持智能查询自动填充, 帮助管理员管理表项

### 维护 token_price_table 实现流程

* [token_price_table](./doc/实体关系表.md#代币币价表--tokenpricetable) 用于存储一段时期内代币的法币价格信息
* 币价为最终值, 其数据源可以来源于交易所或预言机等聚合
* 对于数据源单一, 比较波动大的代币, 可以采用平滑算法减小波动
* 一段时间内币价为空时, 可以采用最后更新币价更新到最新币价, 并人工积极寻找可用的数据源
* 对于同一代币的不同表现, 可以采用同一数据源, 但需要记录在不同表: 如 ETH 和 WETH 需要分别生成两条记录,
  因为对于同一代币的不同表现, 可能会有不同的数据, 如某链上 xBTC 产品价格可能和 BTC 价格脱锚.

### 维护 token_transfer_table 实现流程

* [token_transfer_table](./doc/实体关系表.md#资产转移表--tokentransfertable) 用于储存全量账户转移记录
* 对于大部分 EVM 链:
    * 通过搜集 transaction, trace 信息可以获取主币转移信息
    * 通过收集 Transfer 事件信息, 可以获取代币转移信息
    * 对于特殊的代币合约(如加密猫), 需要监听相应的 Event 事件
* 对于 BTC 链:
    * 解析交易中 input, output 信息可以获取主币转移信息
    * BTC 代币主要为铭文系列 (TODO)
* 对于 SOL 链: **有待调研**
* 对于 TRON 链: **有待调研**
* 交易记录的获取需要逐块获取, 初始化时, 可以从中间任一区块(安全)高度启动, 向两端同步区块数据,
  使得新数据能尽快展示, 并且能逐步恢复旧数据
* 向新数据端同步时, 注意要处理区块校验问题, 对于区块 B(n), 要检查其 parent_hash 是否与库中区块 B(n-1) 的 hash 相同,
  否则库中区块 B(n-1) 数据为失效数据, 应当移除其相关数据.
* 向旧数据端同步时, 不需处理区块校验问题, 单有可能因为数据容量问题, 需要具备移除某个时间段之前数据的能力.

### 维护 account_amount_cache_table 实现流程

* [account_amount_cache_table](./doc/实体关系表.md#用户资产缓存表--accountamountcachetable) 用于缓存用户最新 (block最高高度)
  代币持有数量. 此表只做缓存, 实际资产价值以链上实时结果为准
* 主动更新 account_amount_cache_table 场景如下:
  * 在维护 token_transfer_table 的服务同时, 收集有资产变动的用户地址信息 (chain, token_address, account_address, nft_id), 
    将信息加入**待更新资产地址信息集合**, 以便异步更新.
  * 资产信息更新服务, 定期从**更新资产地址信息集合**获取地址信息, 从链上实时获取相应资产 amount 后, 更新到 account_amount_cache_table
  * 资产信息更新服务异步更新的方案, 一方面可以实现查询周期内地址去重的效果, 可以减少活跃地址重复查询的情况; 
    另一方面, 以 EVM 链为例, 可以通过 multicall 等手段, 批量获取用户资产信息, 以加快更新速度.
  * 对于部分链(如 Sui), 会在交易数据中给出用户地址当前某一代币持有量, 此类链可以在搜集资产变动的同时, 将资产amount一并加入**待更新资产地址信息集合**
  资产信息更新服务可直接采用收集结果.
  * 主动更新依赖于维护 token_transfer_table 的服务, 在实践时通常会因为但不限于以下原因, 发生部分资产变更信息没有及时搜集的情况:
    * 链没有在 api 返回结果中给出所有资产变化的 address, 或者实现过于复杂, 如解析 internal transaction, 或 CFX 链的 eSpace 和主网的关系等.
    * 链交易过程中有隐含的资产转移情况 (如 evm L2 链的交易在 L1 的资产变化情况是否能搜集还有待调研, 
    如抽象账户或其它一些代付机制产生的资产变化情况是否能搜集还有待调研)
    * EVM 链, 合约代码问题导致资产没有相应 Event 产生, 或 indexed 位置错误导致解析错误.
    * EVM 链, 合约 Event 不是标准情况且没有实现特定解析处理, 如 crypto kitty 不是标准 ERC721 合约.
    * 维护 token_transfer_table 的服务没有及时实现交易解析的场景, 如 matic 链的主币转移内部交易会产生特定的 Event, 
    较其它 evm 链而言需要单独开发处理.
* 被动更新主要是对主动更新不足的补充, 主要触发场景为 "展示 address token balance 实现流程"中, 
  "实时获取链上 address 对应 amount 数据" 的情况. 在获取链上信息的之后, 可以查询结果更新到 account_amount_cache_table 中.
  除了以上场景, 还可以提供相应的 api 接口触发特定的地址资产更新

### 与区块链交互 rpc 提升稳定性方案

* 对于重点和高频访问区块链, 可以采用自建节点的方案, 如自建多节点, 可以利用负载均衡方式将其并联起来.
  如有必要, 可以采购商用节点, 处于成本考虑, 可以将商用节点作为自建节点的备份.
* 对于次要链或重要但自建成本较大的链(如BTC), 可以采购商用节点和数据源提供商, 如有必要可采购多家服务, 并将其进行串或并连.
* 节点的管理可依赖区块高度进行筛选, 以官方公开节点或大多数节点提供的区块高度为准, 将区块高度相差过大的节点剔除在服务外并报告异常情况

------------------------------------------------------------

## 参考文档

* [实体关系表](./doc/实体关系表.md)
* [OneKey Final 测试题目](https://chip-asphalt-ca1.notion.site/OneKey-Final-c60da4846a45428e9340749a124e4f11)

## Q&A

### 从头开始同步数据，大概需要多久能完成？是否有什么办法可以加快？

* 以 EVM 链 Transfer Event 为例数据从头开始, 需要几天到十几天的时间能同步完成.
  在本方案设计中, 根据钱包产品的特点, 数据不是从头开始同步, 而是从某一 confirmed 的区块开始,
  向两端进行同步(详见"维护 token_transfer_table 实现流程").
* 同时根据产品特点, 不需要同步所有数据, 即可提供服务. 同时也可以不存储所有数据,
  作为钱包产品使用场景, 例如可以以滑动窗口的形式, 管理和存储最近一至两年数据即可.
* 加快同步的方式之一, 就是将过程拆分为"从节点取数据", "数据计算与格式化", "向数据库存数据" 即"取","算","存"三个流程,
  将三个流程进行流水化处理, 使得在存Block(n)的数据同时可以计算Block(n+1)的数据,并且同时还能拉取Block(n+2)的数据,
  根据实践经验, 流水线瓶颈流程通常为"存". (javascript 虽然是脚本语言, 但在 V8 引擎和 jit 加持下, 完全能够胜任此数据处理工作🕶!)

### 在不分库分表的情况下，数据能全部灌入数据库里面吗？

* 在不分库分表的情况下, 理论上是可以全部将数据灌入数据库的, 但是实践不推荐这样做, 否则会给数据库带来较大插入和读取压力.
* 在实践中, 需要进行好实体关系梳理, 依照数据库范式设计好数据表结构, 以最大程度消除数据冗余性.
* 以 transfer 历史记录为例, 推荐依据不同的链进行分库,
  每个库中再依照不同的 token 属性如"主币","ERC20代币","ERC721代币","ERC1155代币"进行拆表.

### 由于每次插入数据都需要处理索引，那么随着数据量的变大，插入速度会变慢，整个同步时间就会变长，这应该如何处理呢？

* 基于B+Tree建立索引的数据库存在"随着数据量的变大，插入速度会变慢"的情况, 插入和查询速度都会受到影响.
* 为了缓解此类上述情况, 可以根据插入和查询场景选用合适的数据库类型
  基于钱包类产品交易历史的查询场景, 即大部分查询都集中在最近数据插入的数据段, 可以采用基于 LSMTree 进行索引的数据库.
  如果需要对交易历史有较多的点查, 基于 LSMTree 的数据库会使查询效率更加糟糕, 所以分析类事务可以采用列数据库.
* 但是无论哪一类数据库, 都存在插入和查询效率随着数据量的增大而下降的情况, 所以应当合理的规划数据内容, 控制数据规模.
* 采购性能更高的数据库服务也是增强数据存取效率的重要手段!

### token_data_table 这个是存储代币的基本信息, 但是这里和地址并没有任何关系, 在这个模型里面, 如何快速筛选某个用户地址所持有的代币?

* token_data_table 表旨在存储经过选定的代币信息, 不储存用户相关信息
* 目前是通过在 token_data_table 以 (chain,token_address) 为键 join account_amount_cache_table,
  以获取选定范围内的 account_address 持有的所有代币

### 缓存表能否细说一下, 对应数据更新时机是什么? 怎么保证这个缓存表里面的余额是最新的?

* 缓存表及其更新机制详见"维护 account_amount_cache_table 实现流程"
* 由于**主动更新**章节陈述的原因, 主动更新机制无法保证**缓存表里面的余额是最新的**
* 由于区块回滚或节点间时差等原因, **被动更新**机制亦无法保证**缓存表里面的余额是最新的**
* account_amount_cache_table 只能实现数据的大致准确, 是在查询性能和数据准确性之前取舍的实践方案
